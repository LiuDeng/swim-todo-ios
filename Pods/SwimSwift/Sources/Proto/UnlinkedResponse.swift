...
public struct UnlinkedResponse: Envelope, Equatable {
  public let node: Uri
  public let lane: Uri
  ...
  public init?(value: Value) {
    ...
    if heading.isAttr && heading.key?.text == "unlinked", let headers = heading.value.record {
      var node = nil as Uri?
      var lane = nil as Uri?
      var i = 0
  ...
  public func withNode(node: Uri, lane: Uri) -> UnlinkedResponse {
    return UnlinkedResponse(node: node, lane: lane, body: body)
  }

  public func withNode(node: Uri) -> UnlinkedResponse {
    return UnlinkedResponse(node: node, lane: lane, body: body)
  }
  ...
  public var reconValue: Value {
    ...
    var record = Record(Attr("unlinked", Value(heading)))
...
public func == (lhs: UnlinkedResponse, rhs: UnlinkedResponse) -> Bool {
