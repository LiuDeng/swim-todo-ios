...
public struct UnlinkRequest: Envelope, Equatable {
  public let node: Uri
  public let lane: Uri
  ...
  public init?(value: Value) {
    ...
    if heading.isAttr && heading.key?.text == "unlink", let headers = heading.value.record {
      var node = nil as Uri?
      var lane = nil as Uri?
      var i = 0
  ...
  public func withNode(node: Uri, lane: Uri) -> UnlinkRequest {
    return UnlinkRequest(node: node, lane: lane, body: body)
  }

  public func withNode(node: Uri) -> UnlinkRequest {
    return UnlinkRequest(node: node, lane: lane, body: body)
  }
...
public func == (lhs: UnlinkRequest, rhs: UnlinkRequest) -> Bool {
